// background.js - v7.0 (No ID, New Field Map)
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "sendBatchToAnki") {
    ...
    const NOTE_TYPE = "NotebookLM-Quiz-v1"; 
    ...
    const notes = request.batchData.map(card => {
      return {
        ...
        "fields": {
          ...
          "Hint": card.hint,
          // "ArchDiagram" is left empty for manual filling
    ...
    .then(data => {
      if (data.error) {
         ...
         if (successCount === 0 && notes.length > 0) {
             sendResponse({ success: false, error: "Import failed. Check Anki Field names!" });
         } else {
             sendResponse({ success: true, count: successCount });
         }
      }
    })
    .catch(err => sendResponse({ success: false, error: "Connection Failed: Is Anki open?" }));
